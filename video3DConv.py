...
def main(_):
    ...
    frmSize = (112,128)
    # define the dataset
    ucf101data = ucf101(frmSize = frmSize,numOfClasses = numOfClasses)
    ...
    W_fc1 = weight_variable([frmSize[0]/16 * frmSize[1]/16 * 512, 4096])
    b_fc1 = bias_variable([4096])
    
    h_pool2_flat = tf.reshape(h_pool4, [-1, frmSize[0]/16 * frmSize[1]/16 * 512])
    ...
    with sess.as_default():
        ...
        with tf.device("/gpu:0"):
            ...
            for i in range(1000):
                train_x,train_y = ucf101data.loadTrainBatch(30)
                print("i = ", i)
                if i%5 == 0:
